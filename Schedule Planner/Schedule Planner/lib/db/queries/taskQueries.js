...
export async function rebuildReminderJobsForUser(db, userId, tasks, { leadMinutes = 5, integrationId = "gmail" } = {}) {
  const normalizedLeadMinutes = Number.isInteger(leadMinutes) && leadMinutes >= 0 ? leadMinutes : 5;
  ...
  for (const task of tasks) {
    ...
    await db.query(
      ...
      `,
      [userId, task.id, safeIntegrationId, task.date, task.start, timezone, normalizedLeadMinutes]
    );
  }
}
